...
function customize_menu_ui_k2plus() {
  top_line
...
function customize_menu_k2plus() {
  clear
  customize_menu_ui_k2plus
    ...
    case "${customize_menu_opt}" in
      1)
        if [ -f "$BOOT_DISPLAY_FILE" ]; then
          ...
          run "install_custom_boot_display" "customize_menu_ui_k2plus"
        fi;;
      2)
        if [ ! -f "$BOOT_DISPLAY_FILE" ]; then
          ...
          run "remove_custom_boot_display" "customize_menu_ui_k2plus"
        fi;;
      3)
        if [ ! -d "$FLUIDD_FOLDER" ] && [ ! -d "$MAINSAIL_FOLDER" ]; then
          ...
          run "remove_creality_web_interface" "customize_menu_ui_k2plus"
        fi;;
      4)
        if [ -f "$CREALITY_WEB_FILE" ]; then
          ...
          run "restore_creality_web_interface" "customize_menu_ui_k2plus"
        fi;;
      5)
        if [ -d "$GUPPY_SCREEN_FOLDER" ]; then
          ...
          run "install_guppy_screen" "customize_menu_ui_k2plus"
        fi;;
      6)
        if [ ! -d "$GUPPY_SCREEN_FOLDER" ]; then
          error_msg "Guppy Screen is not installed!"
        else
          run "remove_guppy_screen" "customize_menu_ui_k2plus"
        fi;;
      7)
        if [ -f "$FLUIDD_LOGO_FILE" ]; then
          ...
          run "install_creality_dynamic_logos" "customize_menu_ui_k2plus"
        fi;;
      B|b)
